...
class GraspDB(object):
    ...
    def __init__(self):
        ...
        self.dbconnection = mdb.connect("localhost", "root", "", "freegrip")#jiayao inclue drop
    ...
    def loadDropStablePos(self, objname, handname = "rtq85"):
        """
        load self.dropstablepos, etc. from mysqldatabase

        :param handname which hand to use, rtq85 by default
        :return: a list of [iddrop,rot,pos]
        ...
        sql = "SELECT freegrip.dropstablepos.iddropstablepos,\
                ...
                WHERE freegrip.dropstablepos.idobject = object.idobject AND object.name like '%s'" % (objname)

        data = self.execute(sql)
        if len(data) != 0:
            for i in range(len(data)):
                dropid.append(int(data[i][0]))
                rot.append(dc.strToMat3(data[i][1]))
                pos.append(dc.strToV3(data[i][2]))
            return [dropid, rot, pos]
        else:
    ...
    def loadDropFreeGrip(self, objname, handname="rtq85",idstablepos=1):
        ...
        if len(data) != 0:
            ...
            return [freegripid, freegripcontacts, freegripnormals, freegriprotmats, freegripjawwidth,freeairgripid]
        else:
    ...
    def __del__(self):
        ...
        class GraspDB(object):
            ...
            def __init__(self):
                # self.dbconnection = mdb.connect("localhost", "weiweilab", "weiweilab", "freegrip")
                self.dbconnection = mdb.connect("localhost", "root", "", "freegrip")  # jiayao
                self.cursor = self.dbconnection.cursor()
