...
if __name__=='__main__':
    ...
    def drawhist(shortestregrasplength,startid,goalid):
    ...
    def drawonce(shortestregrasplength,startid,goalid):
        ...
        x = startid
        y = goalid
        plt.scatter(x, y, c=z, s=3, alpha=0.6, vmin=-1, vmax=9,cmap=cm.jet)
        ax.set_xticks([])
        ax.set_yticks([])
        plt.colorbar()


    figtitle = 'regrasp length'
    ...
    ########################
    ax = plt.subplot(512)
    ax.set_title("on fixture basket time18")
    sql = "SELECT pathquick2_fixture.idpathquick2_fixture,pathquick2_fixture.startid,pathquick2_fixture.goalid,pathquick2_fixture.pathnum,pathquick2_fixture.shortestregrasplength FROM pathquick2_fixture"
    result = gdb.execute(sql)
    result = np.asarray(result)
    idpath = [int(x) for x in result[:, 0]]
    ...
    #########################
    ax = plt.subplot(513)
    ax.set_title("on fixture basket time 50")

    # sql = "SELECT pathquick3_fixture_quick.idpathquick3_fixture_quick,pathquick3_fixture_quick.startid,pathquick3_fixture_quick.goalid,pathquick3_fixture_quick.pathnum,pathquick3_fixture_quick.shortestregrasplength FROM pathquick3_fixture_quick"
    # sql = "SELECT pathquick2_fixture.idpathquick2_fixture,pathquick2_fixture.startid,pathquick2_fixture.goalid,pathquick2_fixture.pathnum,pathquick2_fixture.shortestregrasplength FROM pathquick2_fixture"
    sql = "SELECT path4_fixture.idpath4_fixture,path4_fixture.startid,path4_fixture.goalid,path4_fixture.pathnum,path4_fixture.shortestregrasplength FROM path4_fixture"
    result = gdb.execute(sql)
    result = np.asarray(result)
    idpath = [int(x) for x in result[:, 0]]
    startid = [int(x) for x in result[:, 1]]
    ...
    ##############################
    ax = plt.subplot(514)
    ax.set_title("on fixture basket time 100")
    sql = "SELECT path5_fixture.idpath5_fixture,path5_fixture.startid,path5_fixture.goalid,path5_fixture.pathnum,path5_fixture.shortestregrasplength FROM path5_fixture"
    result = gdb.execute(sql)
    result = np.asarray(result)
    idpath = [int(x) for x in result[:, 0]]
    startid = [int(x) for x in result[:, 1]]
