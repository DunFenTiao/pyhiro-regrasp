    ...
    def selectstartik_all(self, startrotmat4, startid, base):
        sql = "SELECT ik_start.idstart,ik_start.idfreeairgrip,ik_start.feasibility, ik_start.feasibility_handx,  \
                        ik_start.feasibility_worlda, ik_start.feasibility_worldaworldz FROM ik_start\
                           ...
                           AND ik_start.feasibility_worlda='True' AND ik_start.feasibility_worldaworldz='True' " % (startid)
        result = self.gdb.execute(sql)
        if len(result) != 0:
            tpsrows = np.array(result)
...
if __name__=='__main__':
    ...
    startid=1
    goalid=9
    startrotmat4 =  Mat4(0.918899595737,-0.307511538267,-0.247103631496,0.0,-0.307511538267,-0.166003227234,-0.936952292919,0.0,0.247103631496,0.936952292919,-0.247103646398,0.0,160.898132324,-185.891098022,6.62031936646,1.0)
    goalrotmat4 =  Mat4(1.0,-0.000112805166282,0.000112921290565,0.0,-0.000112805166282,0.00102891668212,0.999999463558,0.0,-0.000112921290565,-0.999999463558,0.00102890387643,0.0,152.225143433,-196.06300354,-55.009437561,1.0)
