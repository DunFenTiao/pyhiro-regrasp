...
class Freegrip(fgcp.FreegripContactpairs):
    ...
    def __init__(self, objpath, handpkg, idstablepos,readser=False, torqueresist = 50):
        """
        initialization
        ...
        super(self.__class__, self).__init__(objpath, idstablepos ,readser)
        if readser is False:
    ...
    def saveToDB(self, gdb):
        ...
        for i in range(len(self.gripcontacts)):
            ...
            gdb.execute(sql)
        # else:
        #     print "Drop Free Grasps already saved or duplicated filename!"
...
class DropfreeGrip(object):
    ...
    def __init__(self, gdb, objname, handpkg):
        idstablepos=1
        dropfreegripdata = gdb.loadDropFreeGrip(objname, handname = handpkg.getHandName(),idstablepos=1)
        if dropfreegripdata is None:
            raise ValueError("Plan the dropfreegrip first!")

        self.freegripids = dropfreegripdata[0]
...
if __name__=='__main__':
    ...
    handpkg = rtq85nm
    # idstablepos is the one drop in droptimes of object
    idstablepos=1
    freegriptst = Freegrip(objpath, handpkg, idstablepos,readser=False, torqueresist = 50)
    ...
    objgeom = pandageom.packpandageom(workcellmesh.vertices, workcellmesh.face_normals,
                                      workcellmesh.faces)
    workx = 0
    worky = 0
    workz = 0
